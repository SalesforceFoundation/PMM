from cumulusci.core.tasks import BaseTask

class TableLogger(BaseTask):

    def _run_task(self):
        pass

    @staticmethod
    def get_padded_columns(column_max_lengths, column_range, row):
        log_row = []
        for index in column_range:
            column = row[index] if index < len(row) else ""
            log_row.append("{}{}".format(
                column,
                " " * (column_max_lengths[index] - len(column))
            ))
        return log_row

    @staticmethod
    def get_table_borders(column_max_lengths):
        table_borders = {}
        table_borders["separators"] = []

        for i in range(0, len(column_max_lengths) + 1):
            columns = []
            for index, length in enumerate(column_max_lengths):
                columns.append((" " if index < i else "─") * length)

            if i == 0:
                separator_format = "├─{}─" + ("┼─{}─" * (len(column_max_lengths) - 1)) + "┤"
                table_borders["header"] = ("┌─{}─" + ("┬─{}─" * (len(column_max_lengths) - 1)) + "┐").format(*columns)
                table_borders["footer"] = ("└─{}─" + ("┴─{}─" * (len(column_max_lengths) - 1)) + "┘").format(*columns)
            elif i < len(column_max_lengths):
                separator_format = ("│ {} " * i) + "├─{}─" + ("┼─{}─" * (len(column_max_lengths) - 1 - i)) + "┤"
            else:
                separator_format = ("│ {} " * len(column_max_lengths)) + "│"
                table_borders["row_format"] = separator_format
            
            table_borders["separators"].append(separator_format.format(*columns))

        return table_borders

    @staticmethod
    def format_rows(raw_rows):
        formatted_rows = {}
        formatted_rows["rows"] = []
        column_max_lengths = []
        formatted_rows["column_max_lengths"] = column_max_lengths

        if raw_rows:
            for raw_row in raw_rows:
                row = []
                formatted_rows["rows"].append(row)
                for index, raw_column in enumerate(raw_row):
                    column = str(raw_column) if raw_column else ""
                    row.append(column)
                    if index < len(column_max_lengths):
                        column_max_lengths[index] = max(column_max_lengths[index], len(column))
                    else:
                        column_max_lengths.append(len(column))
        return formatted_rows

    def log_table(self, raw_rows, groupByBlankColumns=False):
        formatted_rows = TableLogger.format_rows(raw_rows)
        rows = formatted_rows.get("rows")
        column_max_lengths = formatted_rows.get("column_max_lengths")
        
        if column_max_lengths:
            column_range = range(0, len(column_max_lengths))

            table_borders = TableLogger.get_table_borders(column_max_lengths)

            self.logger.info(table_borders.get("header"))

            #last_group_index = -1
            for index, row in enumerate(rows):
                # log separator
                if 0 < index:
                    separator_index = 0
                    if groupByBlankColumns:
                        separator_index = len(column_max_lengths) 
                        for i in column_range:
                            if row[i]:
                                separator_index = i
                                break
                    self.logger.info(table_borders.get("separators")[separator_index])
                
                # log row
                self.logger.info(
                    table_borders.get("row_format").format(
                        *TableLogger.get_padded_columns(column_max_lengths, column_range, row)
                    ))

            self.logger.info(table_borders.get("footer"))
