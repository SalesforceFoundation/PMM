...
class ServiceDeliveryListingPage(BasePMDMPage, ListingPage):
    object_name = "None"


@pageobject("NewServiceDelivery", "ServiceDelivery__c")
class NewServiceDeliveryPage(BasePMDMPage, BasePage):
    ...
    def _is_current_page(self):
        ...
        self.selenium.location_should_contain("/lightning/o/ServiceDelivery__c/",message="Section title is not 'New Service Delivery' as expected")
    ...
    def populate_new_service_delivery_form(self, **kwargs):
        ...
        for key, value in kwargs.items():
            if key == 'Service Delivery Name':
                locator = pmdm_lex_locators["new_record"]["text_field"].format("Service Delivery Name")
                self.selenium.set_focus_to_element(locator)
                self.selenium.get_webelement(locator).send_keys(value)
            elif key == 'Delivery Date':
                locator = pmdm_lex_locators["new_record"]["open_date_picker"].format("Delivery Date")
                self.selenium.set_focus_to_element(locator)
                self.selenium.get_webelement(locator).click()
                popup_loc = pmdm_lex_locators['new_record']['datepicker_popup']
                self.selenium.wait_until_page_contains_element(popup_loc, error="Date picker for delivery date did not open ")
                value_loc=pmdm_lex_locators["new_record"]["select_date"].format(value)
                ...
                locator = pmdm_lex_locators["new_record"]["text_field"].format("Quantity")
                self.selenium.set_focus_to_element(locator)
                self.selenium.get_webelement(locator).send_keys(value)
            else:
...
class ServiceDeliveryDetailPage(BasePMDMPage, DetailPage):
