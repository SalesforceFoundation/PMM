...
class MappingGenerator(NamespaceInfo, BaseSalesforceApiTask):
    ...
    def get_project_namespace(self):
        return (
            ...
            if self.org_config.config.get("project_namespace")
            else super().get_project_namespace()
        )
        ...
        if option_mapping_configs:
            for mapping_config in option_mapping_configs:
                ...
                if add_mapping_config:
                    if (
                        process_bool_arg(mapping_config.get("inject_project_namespace"))
                        or "namespace" not in mapping_config
                    ):
    ...
    def get_combined_mapping(self):
        ...
        if (
            not process_bool_arg(self.options.get("skip_mapping_project"))
            and self.get_project_namespace()
            not in self.get_installed_package_namespaces()
        ):
...
class LogMapping(MappingGenerator):
    ...
    task_options = {**MappingGenerator.task_options}
